...
export const LargeImageSlider: React.FC<Props> = ({ closeSlider, currentPost }) => {
  ...
  return (
    <div className="fixed top-0 left-0 flex items-center justify-center w-full h-full bg-transparent_white p-8 z-20">
      <button 
        className="w-8 h-8 fixed top-8 right-8 bg-udh_yellow rounded z-30 hover:border-udh_dark_green" 
        onClick={closeSlider}>x</button>
        
      <div className="max-w-4xl w-full bg-udh_green rounded-md relative">
        {currentPost?.mediaType === "CAROUSEL_ALBUM" && (
          <div className="flex items-center justify-center h-full">
            <button 
              className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-udh_yellow rounded p-2 z-30 disabled:opacity-50"
              onClick={prevImage}
            ...
            <div className="overflow-hidden w-full h-full border-4">
              <div
            ...
            <button 
              className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-udh_yellow rounded p-2 z-30 disabled:opacity-50"
              onClick={nextImage}
              // @ts-ignore:
